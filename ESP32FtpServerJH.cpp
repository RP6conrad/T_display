...
void FtpServer::begin(String uname, String pword)
{
  ...
  if(!SD.begin(SDCARD_SS, sdSPI)) //if(!SD.begin())
  {
...
//  if (ftpServer.available()) {
  ...
  {				
    if( cmdStatus == 3 ){ 	// Ftp server waiting for user identity
      if( feat())			//With Android FTP, FEAT command comes before login !!
        cmdStatus = 3;
      else if (userIdentity()) cmdStatus = 4;
			else cmdStatus=0;
...
void FtpServer::clientConnected()
{
  ...
  client.println( "220--- By David Paiva + JH  ---");
...
boolean FtpServer::feat()				//With Android FTP, FEAT comes before login !!!
{	
if( ! strcmp( command, "FEAT" ))
  {
    client.println( "211-Extensions suported:");
    client.println( " MLSD");
	client.println( " MLST type*;modify*;size*;");
...
{	
  ...
  if(parameters)		//check for nullpointer jh
	{
...
{
if(parameters){							//checking for null pointer jh
...
boolean FtpServer::processCommand()
{
  ...
  //                                   //
  ///////////////////////////////////////
  if( ! strcmp( command, "SYST" ))	//Android devices ask for System
	  {   
  ...
  {
      client.println( "500 Command not recognized");
  }
  ...
  else if( ! strcmp( command, "PASV" ))
  {
	...
	if(WiFi.localIP().toString()=="0.0.0.0")//JH 17/8/2021, om te kunnen werken met softAP !!!
		 dataIp = WiFi.softAPIP();	
	else dataIp = WiFi.localIP();	  
	//dataIp = WiFi.localIP();	
	dataPort = FTP_DATA_PORT_PASV;
...
//      if( !SD.exists(cwdName))
  ...
  //
  else if( ! strcmp( command, "MLSD" )| ! strcmp( command, "MLST" ))
  {
    if( ! dataConnect())
    {
