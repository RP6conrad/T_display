...
void Bat_level_Simon(int offset) {
  ...
  int batW = 10;
  int batL = 25;
  int posX = 246 - batW - 6;//was -10
  int posY = 135 - batL;
  int line = 2;
  int seg = 3;
  int segW = batW - 2 * line;
  int segL = (batL - 0.25 * batW - 2 * line - (seg - 1)) / seg;
  tft.fillRoundRect(offset + posX, posY, 0.5 * batW, 0.25 * batW, 0,TFT_GREEN);                 //battery top
  tft.fillRoundRect(offset + posX - 0.25 * batW, posY + 0.25 * batW, batW, batL,0, TFT_GREEN);  //battery body
  ...
  if (bat_perc < 1) tft.fillRoundRect(offset + posX - 0.25 * batW + line, posY + 0.25 * batW + line + 2 * (segL + 1), segW, segL,0, TFT_BLACK);
...
void Sats_level(int offset) {
 ...
 // int circelS = 2;
  ...
  //display.setFont(&FreeSansBold9pt7b);
  tft.setCursor(posX - (satnum < 10 ? 9 : 18), 114);
  tft.print(satnum);
}
int Time(int offset) {
  if (!update_time()) {
...
void Screen_Init_GPS(void){
  if(config.ublox_type==0xFF){
      ...
      tft.println("");
      if(ubxMessage.navPvt.numSV<5){tft.print("Waiting for Sat>=5");}
      else{
        tft.println("Ready for action");
        tft.print("Move faster than ");
        if ((int)(calibration_speed * 100000) == 194) {
          tft.print(config.start_logging_speed * 1.94384449);
          tft.print("kn");
        }
        if ((int)(calibration_speed * 1000000) == 3600) {
          tft.print(config.start_logging_speed * 3.6);
          tft.print("km/h");
        }
      }
    }
...
void GPS_Screen(void){
   ...
   tft.setCursor(0,0);
   tft.print("R ");
   tft.print(S10.s_max_speed * calibration_speed);
   tft.print(" avg ");
   tft.println(calibration_speed,S10.avg_5runs * calibration_speed);tft.print("      ");
   tft.setCursor(0,25);
   tft.setTextFont(8);
   tft.print(gps_speed * calibration_speed);
   tft.setTextFont(4);
   //"Run","Avg ",S10.s_max_speed * calibration_speed,S10.avg_5runs * calibration_speed,gps_speed * calibration_speed,0);
}  
#else
//****************************************************************************************************************************************************
...
void Bat_level(int X_offset, int Y_offset) {
  ...
  tft.fillRoundRect(X_offset + 3, Y_offset, 6, 3, 0,TFT_WHITE);
  //fillRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t radius, uint32_t color);
  tft.fillRoundRect(X_offset, Y_offset + 3, 12, 30,0, TFT_WHITE);  //monitor=(4.2-RTC_voltage_bat)*26
  if (RTC_voltage_bat < VOLTAGE_100) {
    bat_symbol = (VOLTAGE_100 - RTC_voltage_bat) * 28;
    tft.fillRoundRect(X_offset + 2, Y_offset + 7, 8, (int)bat_symbol,0,TFT_BLACK);
  }
}
...
void Update_screen(int screen) {
  ...
  if (screen == STATS6) {  //Simon stat screen
    ...
    display.setCursor(col4, row2);
    display.println(S10_previous_run * calibration_speed, 2);//previous run
    display.setCursor(col4, row3);
